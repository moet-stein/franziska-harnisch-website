...
export default function PictureHometwo({ pic }) {
    ...
    return (<Grid container style={{ width: "100%" }} >
        <Grid item xs={12} sm={4} style={{ paddingLeft: 30, display:"flex", flexDirection:"column-reverse" }}  >
            ...
            <img style={{ borderRadius: 5, width:"80%" }} onMouseOver={() => setExit(prev => !prev)} className={exit ? classes.imageMask : classes.imageNoMask} src={typeof pic[0].image === 'string' ? pic[0].image : pic[0].image.childImageSharp.fluid.src} />
