...
export default function PictureHometwo({ pic }) {
    ...
    return (<Grid container >
        <Grid item style={{ position: "relative", top: 20 }}>
            {typeof pic[0].image === 'string' ? (
                <img style={{ borderRadius: 5 }} onMouseOver={() => setExit(prev => !prev)} className={exit ? classes.imageMask : classes.imageNoMask} src={pic[0].image} />
            ) : (
        ...
        <Grid item style={{ position: "relative", top: -100, left: 120 }}>
            {typeof pic[1].image === 'string' ? (
                <img style={{ borderRadius: 5 }} onMouseOver={() => setVisible(prev => !prev)} className={visible ? classes.imageMask : classes.imageNoMask} src={pic[1].image} />
        ...
        <Grid item style={{ position: "relative", top: 80, left: 190 }}>
            {typeof pic[2].image === 'string' ? (
                <img style={{ borderRadius: 5 }} onMouseOver={() => setThirdPic(prev => !prev)} className={thirdPic ? classes.imageMask : classes.imageNoMask} src={pic[2].image} />
